...
class RemoteRepository(object):
    ...
    def __init__(self, repository, git, git_url_parser, clone_path, log):
        ...
        :param repository: The repository as an URL to a
            git repository
        :param git: The Git object used to run git commands.
        :param git_url_parser: Parser to extract information from
            the git URL
    ...
    def _unique_name(self):
        ...
        git_info = self.git_url_parser.parse(url=self.repository)

        url_hash = hashlib.sha1(
            self.repository.encode('utf-8')).hexdigest()[:6]
        return git_info.name + '-' + url_hash

    def _repository_path(self):
        """ :return: The path where we clone the repository """

        return os.path.join(self.clone_path, self._unique_name())
