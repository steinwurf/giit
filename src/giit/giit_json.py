...
class GiitJson(object):
    ...
    def _from_branch(self):
        self.log.info("Using giit.json from branch %s",
                      self.config_branch)

        self.git_repository.checkout_branch(
            remote_branch=self.config_branch)
    ...
    def _from_workingtree(self):
        ...
        config_path = os.path.join(
            self.git_repository.workingtree_path(), 'giit.json')

        self.log.info("Using giit.json from workingtree %s",
                      config_path)
